...
const OrgForm = (props) => {
    ...
    const { register, handleSubmit, formState: { errors } } = useForm({
        ...
        defaultValues: { 
            ...
            assetRole: organisation.assetRole,
            inuse: organisation.inuse
        }
    });

    const [ status, setStatus ] = useState(organisation.inuse);

    const onSave = useCallback((data) => {
        save({ ...data, uid: organisation._id });
        toggle();
    }, [organisation._id, save, toggle]);
